...
#ifdef SILO
struct DBfile ;
int TimePlotCurve_post_read(struct DBfile *idbid, TimePlotCurve_t *ptr, char *field_name);
int TimePlot_post_read(struct DBfile *idbid, TimePlot_t *ptr, char *field_name);
int TimePlot_write(struct DBfile *idbid, TimePlot_t *ptr, char *field_name);
#endif

TimePlotCurve_t * TimePlotCurve_construct(int num);
TimePlotCurve_t * TimePlotCurve_create(char *sym, int tpc_num);
TimePlotCurve_t * TimePlotCurve_destruct(TimePlotCurve_t *ptr);
TimePlotCurve_t * free_TimePlotCurve_t(TimePlotCurve_t *tp);
TimePlot_t * TimePlot_construct(int num);
TimePlot_t * TimePlot_destruct(TimePlot_t *ptr);
TimePlot_t * free_TimePlot_t (TimePlot_t *tp) ;
...
------------------------------------------------------------------------------*/
        ...
        - tpdata[] 
        ----------------------------------------------------------------------*/
int ntp ;		/* Current num of time plot data structures */
int maxtpdata ;		/* Maximum num of time plot data structures.  
				   This is therefore the number of entries
				   allocated for the global array tpdata[]. 
				   As of 09/10/97, this is set to 20 in
				   parminit.c */
int itp ;		/* This is used as an index into the time 
				   plot array tpdata[] */

int maxtplot;		/* Max number of tplot lines within a */
...
int maxtimeplot ;
	...
	- time plot structure
	-----------------------------------------------------------------------*/
TimePlot_t **tpdata ;    /* Array of ptrs to time plot data structs */
