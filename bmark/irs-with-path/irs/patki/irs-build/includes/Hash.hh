...
HashElm_t * * hash_get_elm_list(int *num_elms, char *type, HashTable_t *ht);
HashElm_t * hash_lookup_elm (char *name, HashTable_t *ht);
HashTable_t * hash_tbl_create(int size, char *name);
HashTable_t * hash_tbl_free(HashTable_t *ht, int free_obj_flag);
void * hash_lookup_obj (char *name, HashTable_t *ht);
void hash_add(char *name, char *type, void *obj, HashTable_t *ht);
void hash_cond_print_by_type(HashTable_t *ht, char *name);
void hash_del(char *name, HashTable_t *ht);
void hash_del2(char *name, char *type, HashTable_t *ht);
void hash_del_objs_of_type(HashTable_t *ht, char *type, void (*fn_ptr) ());
void hash_print        (HashTable_t *ht);
void hash_print_by_name(HashTable_t *ht);
void hash_print_by_type(HashTable_t *ht);
void hash_print_obj(HashElm_t *ptr);
void hash_print_structs_of_type(HashTable_t *ht, char *type);
void hash_sort_array(HashElm_t **hash_elm_array, int num_objs);
void hash_sort_array_by_type(HashElm_t **hash_elm_array, int num_objs);
void hash_analyze(HashTable_t *ht, int print_flag, FILE *fp, double *avg_chain, int *max_chain);
int get_num_elms_in_hashtable(HashTable_t *ht);
int hash_optimize(HashTable_t * *htp);
...
HashTable_t *gv_hash_tbl =  NULL ;
HashTable_t *gv_perm_ht =  NULL ;
