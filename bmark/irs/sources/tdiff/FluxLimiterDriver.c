...
void FluxLimiterDriver( double *x, double *y, double *z, double *phi,
                        double *flmt, double fscl, double umin, Domain_t *domain )
{
   char *me = "FluxLimiterDriver" ;
   int i, j ;
 ...
 {       imin[6] = domain->imin     ;
   ...
   if ( ndims == 2 ) {
      FluxLimiterPert2d( xc, yc, domain, phi, flmt, umin, fscl,
		         imin[6], imax[6], jmin[6], jmax[6] ) ;
   }
   if ( ndims == 3 ) {
      FluxLimiterPert3d( flmt, phi, xc, yc, zc, volc, domain, fscl, umin,
		         kmin[6], kmax[6], jmin[6], jmax[6], 
		         imin[6], imax[6] ) ;
   }
