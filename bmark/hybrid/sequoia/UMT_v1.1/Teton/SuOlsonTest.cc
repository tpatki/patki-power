...
{
    int myRank=0, numProcs, theOrder=12, theNumGroups=16;
    ...
    if( argc >= 4 )
    {
        theNumGroups = atoi(argv[3]);
        if(myRank == 0)
            cout<<" setting the number of groups="<<theNumGroups<<endl;
    }
    
    if(myRank == 0)
        cout<<" theNumGroups = "<<theNumGroups<<"  theOrder= "<<theOrder<<endl;
    ...
    cout<<" SMS index for task "<<myRank<<":("<<sms_i<<","<<sms_j<<","<<sms_k<<")."<<endl;
...
// #ifdef BGP
    ...
    initialize(myMesh, myTetonObject, myPartList, theOrder, theNumGroups);
    if(myRank == 0)
        cout<<"    done."<<endl;
                ...
                int theOrder, int theNumGroups)
{
    ...
    std::vector<TetonBoundary> boundaryData;
    ...
    CharStar8 theSrcName, theSrc,theShape;
    strncpy(theSrcName.data,"Source  ",8);
    strncpy(theSrc.data,"temp    ",8);
    strncpy(theShape.data,"iso     ",8);
    
    TetonVolumeSource theVolSource(theSourceZones, theSrcName, theSrc, theShape, 1.0, theTimeTemps);
    volSourceData[0]=theVolSource;

    // set group boundaries
    setFrequencies( theNumGroups, theOrder, freqData);
    ...
    for(int i=0;i<bcNames.size()-1; i++)
    {
        bcNames[i] = "Refl    ";
        srcNames[i] = "refl    ";
    }
    bcNames[bcNames.size()-1] = "Vac     ";
    srcNames[bcNames.size()-1] ="vac     ";
    ...
    theTeton.linkKull(myMesh, freqData, boundaryData, volSourceData, PXlookup);
