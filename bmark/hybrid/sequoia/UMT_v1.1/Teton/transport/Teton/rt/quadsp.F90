...
!  Local
   ...
   integer    :: i,nangl,ns,nf,np,nordp1,norder
...
!  MU < 0
   ...
   do i=ns,nf
      self% omega(1,i-np+1) = -amu(i)
     self% weight(i-np+1)   =  wgt(i)
   enddo
 
!  MU > 0
 
   do i=nangl,1,-1
      self% omega(1,nordp1-i+1) =  -self% omega(1,i+1)
     self% weight(nordp1-i+1)   =  self% weight(i+1)
   enddo
