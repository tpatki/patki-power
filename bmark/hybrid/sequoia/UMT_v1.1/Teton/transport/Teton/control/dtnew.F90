...
!                                                                      *
!***********************************************************************
   subroutine dtnew(Size, Geom, Mat, DtControls, IterControls)
...
!  Local
   ...
   integer    :: iz,izdtex,izdtr4x,noutrt,noutmx,ninrt,ninmx,noutclose
...
!  We test on the radiation energy density and electron temperature
   ...
   threshold = cutoff*tr4max*Geom%volz(iztr4max(1))

   call MPIAllReduceT(THRESHOLD, "max", MPI_COMM_WORLD)

   ZoneLoop: do iz=1,Size%nzones
     ...
     if (tr4*Geom%volz(iz) > threshold) then
