...
!***********************************************************************
   ...
   subroutine rtbdry(Size, Geom, Prof, PHINC)
...
!  Arguments
   ...
   real(adqt), intent(inout) :: phinc(Size%ngr,Size%nangSN,0:Prof%NumProfiles)
 
!  Local

   type(Profile),    pointer :: ProfID

   integer    :: ia,ig,ip,ngr,nangSN
...
!***********************
       ...
       if (Type == 'temp' .or. Type == 'fds') then
         ...
         do ia=1,nangSN
           phinc(:,ia,ip) = wtiso*smult(:,ia)*ProfID% InterpValues(:)
         enddo
