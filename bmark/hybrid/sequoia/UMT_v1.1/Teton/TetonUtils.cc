...
void setBoundary( const MeshBase& mesh, std::vector<TetonBoundary>& theBoundary, 
...
#pragma omp parallel private(myThreadNum)
    ...
    assert( bcNames.size() == numTags );
    assert( bcNames.size() == numTags );

    std::map<std::string,std::vector<int> > problemBoundaryFaceIDs;
         ...
         fIt != mesh.ownedFaceEnd(); ++fIt)
    {
        if( fIt->getFacePtr()->FType == PROBLEM_BOUNDARY_FACE )
        {
            ...
            assert( i<tagNames.size() );

            // get the Boundary Condition type associated with this boundary face tag
            std::string theBCName = srcNames[i];
            
            // add this face id to the vector of boundary faces of BC type 'theBCName'
            problemBoundaryFaceIDs[theBCName].push_back( fIt->getLocalID() );
        }
    }//end of loop over faces

    // loop over list of BC types and create TetonBoundary objects for each 
    for(std::map<std::string,std::vector<int> >::const_iterator it = problemBoundaryFaceIDs.begin();
        it != problemBoundaryFaceIDs.end() ; ++it)
    {
        CharStar8 bcname, bsrce;
        strncpy(bcname.data,it->first.c_str(),8);
        strncpy(bsrce.data, it->first.c_str(),8);
        theBoundary.push_back( TetonBoundary(it->second, bcname, bsrce) );        
    }
    
    
}//end of setBoundary

void setFrequencies( int numgp, int theOrder, std::vector<TetonFreq>& theFreqs )
{
    assert ( (numgp>=1) );
    ...
    if (numgp == 1)
    {
        theFreqs.push_back(TetonFreq(0.0001, 200, LevelSym, Order, Npolar, Nazimu, PolarAxis));
    }
    else if(numgp > 1)
    {
        theFreqs.push_back(TetonFreq(0.0001, 0.01, LevelSym, Order, Npolar, Nazimu, PolarAxis));
        theFreqs.push_back(TetonFreq(0.01  , 0.05, LevelSym, Order, Npolar, Nazimu, PolarAxis));
        for(int g=2;g<numgp;g++)
        {
            theFreqs.push_back(TetonFreq(.05*(g-1), 0.05*g, LevelSym, Order, Npolar, Nazimu, PolarAxis));
        }
    }
...
double advance(MeshBase& theMesh, Teton<MeshBase>& theTeton, PartList<MeshBase> &thePartList)
{
    ...
    theTeton.CupdateSn( thePartList );
