...
static char* libmsrhdr = "AAA Hostname";
...
/* Name of rapl output file.  Only used if -O option is given.  */
static const char *rapl_filename;
...
static struct option longopts[] =
{
  ...
  {"tick-file", required_argument, NULL, 'O'},
  {"portability", no_argument, NULL, 'p'},
  {"enable-turbo", no_argument, NULL, 't'},
  {"disable-turbo", no_argument, NULL, 'T'},
...
/* summarize: Report on the system use of a command.
   ...
   % == a literal `%'
   A == R-friendly header for libmsr interface.
...
*  X == average amount of shared text in K (ru_ixrss)
   Z == page size
   a == results from the libmsr interface.
	...
	case '%':
	  switch (*++fmt)
	    ...
	    case '%':		/* Literal '%'.  */
	      putc ('%', fp);
	      break;
	    case 'A':		/* R-friendly header for msr output. */
	      fprintf (fp, "%s", libmsrhdr);
	      break;
	    ...
	    case 'Z':		/* Page size.  */
	      ...
	      break;
	    case 'a':		/* Output from libmsr */
	      fprintf (fp, "ZZZ %s", resp->hostname);
	      break;
     ...
     char **argv;
{
  ...
  while ((optc = getopt_long (argc, argv, "+af:o:O:ptTvV", longopts, (int *) 0))
	 != EOF)
    {
      switch (optc)
	{
     ...
     char **argv;
{
  ...
  rapl_init(rapl_filename);
  rapl_tick( rapl_filename );
  ...
  enable_all_turbo(); // Turbo is now the default.  Always put it back.
  //dump_turbo();
  rapl_finalize(NULL, 1);
  fflush (outfp);
